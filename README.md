...
| Name | Arguments | Description |
|------|-----------|-------------|
|`showSuggestions`| HTML click event | Fires whenever the input is being clicked. Shows suggestion list (gets them from the source if none is present). Used to be named `onInputClick`. |
...
|`moveSelection`| HTML keydown event | Fires on component keydown. Used for moving list selection. Used to be named `onArrowKeyDown`. |
